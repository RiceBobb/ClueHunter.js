...
export async function clueHunt(answer: string, parsed_text: string, searching_passage_limit: number = 50) {
    const passages = wink_splitter(parsed_text);
    const searched_passages_doc = await bm25_search(answer, passages, searching_passage_limit);
